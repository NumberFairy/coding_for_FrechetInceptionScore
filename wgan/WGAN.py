...
class WGAN(object):
    ...
    def train(self):
        ...
        # #存储整个样本的均值和协方差
        # mu_x = np.mean(self.data_X,  axis=0)
        # mu_x = np.reshape(mu_x, (28, 28))
        # data = self.data_X
        # data = np.reshape(data, (data.shape[0], -1))
        # imxu_x = np.cov(data, rowvar=False)
        # np.savez('npy/' + self.model_name + self.dataset_name + '.npz', mu_x=mu_x, imxu_x=imxu_x)
        for epoch in range(start_epoch, self.epoch):
            ...
            # 这里可以不用生成每一步的结果，最后训练完成后再统一生成最好的结果（这里只是为了对比每一次迭代的效果罢了）
    ...
    def visualize_results(self, epoch):
        ...
        for i in range(500):
            z_sample = np.random.uniform(-1, 1, size=(self.batch_size, self.z_dim)) #（64， 62）
            ...
            for sample in samples:
                ...
                scipy.misc.imsave(check_folder(
                             ...
                             '_epoch%03d' % epoch + '_%d' % i + '_count%d' % count + '_test_all_classes.png', sample)
